...
def get_tasks_keys_for_user(owner_id):
    keys = get_keys_by_pattern(f"*{owner_id}_*")
...
def get_tasks_data_for_user_by_type(owner_id, task_type):
...
def get_tasks_data_for_user(owner_id):
    keys = get_tasks_keys_for_user(owner_id)
    data = [get_task_data_by_key(key) for key in keys]
